...
do_check() {
    ...
    local upstream=$(get_upstream "$stage")
    local output_pattern=$(get_output "$stage")
...
do_pass() {
    ...
    get_review_pattern() {
        case "$1" in
            prd)     echo "*需求评审报告*" ;;
            arch)    echo "*架构评审报告*" ;;
            detailed) echo "*详设评审报告*" ;;
            review)  echo "*代码评审报告*" ;;
            *)       echo "" ;;
        esac
    }
    local review_pattern=$(get_review_pattern "$stage")
...
do_status() {
    ...
    for s in prd arch detailed code review; do
        ...
        local output_pattern=$(get_output "$s")
        local has_output=false
